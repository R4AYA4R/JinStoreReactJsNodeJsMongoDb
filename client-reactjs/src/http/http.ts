...
$api.interceptors.request.use((config) => {
    ...
    config.headers.Authorization = `Bearer ${localStorage.getItem('token')}`; // указываем значение полю Authorization у поля headers у config,даем ему значение токена(для accessToken в данном случае),первым словом указываем тип токена Bearer(в данном случае) и сам токен,который берем из localStorage по ключу(по названию) token
...
$api.interceptors.response.use((config) => {
    ...
    // если status код у response у error равен 401(то есть пользователь не авторизован), и если error.config true(то есть error.config есть),и если error.config._isRetry false(то есть повторный запрос мы еще не делали,это мы проверяем,чтобы эти запросы не зациклились и чтобы повторный запрос можно было сделать только 1 раз)
    if(error.response.status === 401 && error.config && !error.config._isRetry){
        ...
        try{
            ...
            const response = await axios.get<AuthResponse>(`${API_URL}/refresh`,{withCredentials:true}); // в переменную response здесь будут помещены 2 токена(access и refresh) и поле user с типом данных на основе нашего интерфейса IUser,если запрос прошел успешно
