...
const ProductItemPage = () => {
    ...
    const { mutate } = useMutation({
        ...
        onSuccess() {
            ...
            // refetchComments(); // не указываем здесь отдельно повторный запрос на получение массива комментариев,так как при изменении состояния текущей страницы пагинации page в useEffect указали этот повторный запрос на сервер для получения массива комментариев,то есть когда мы тут изменяем состояние page на 1,то автоматически сразу идет повторный запрос на получение массива комментариев и этот дополнительный делать не нужно
        }
