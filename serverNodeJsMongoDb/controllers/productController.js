...
class ProductController {
    ...
    async getAllProductsCart(req, res, next) {
        ...
        try {
            ...
            const productsCart = await cartProductModel.find({ forUser: userId }); // получаем все объекты товаров корзины у которых поле forUser равно параметру userId который мы взяли из query параметров url(строки) запроса,то есть получаем все объекты товаров для определенного авторизованного пользователя

            return res.json(productsCart); // возвращаем найденные товары корзины на фронтенд
