...
class ProductController {
    ...
    async createNewProductCatalog(req, res, next) {
        ...
        try {
            ...
            const mainImagePathStatic = `${path.resolve()}\\static\\(${randomStringMainImage})${productCatalog.mainImage}`; // указываем нашу переменную randomStringMainImage перед названием картинки productCatalog.mainImage,чтобы картинка потом правильно сохранилась и имела уникальное название

            const newMainImage = `(${randomStringMainImage})` + productCatalog.mainImage; // создаем переменную для нового названия главной картинки для товара,добавляем к основному названию картинки(productCatalog.mainImage) в начало рандомную уникальную строку randomStringMainImage(в скобках ее указываем просто чтобы лучше смотрелось),чтобы каждая картинка в папке static имела рандомное уникальное название,чтобы можно было добавлять картинки с одинаковыми названиями на компьютере,но с разными значениями этих картинок,иначе,даже если картинки разные,но их названия уже есть в папке static,то будет ошибка,что такой файл есть,хотя картинки разные,поэтому делаем так
            ...
            const newDescImages = []; // создаем переменную newDescImages для нового массива картинок описания
            ...
            productCatalog.descImages.forEach(image => {
                ...
                fs.cpSync(`${path.resolve()}\\checkStatic\\${image}`, `${path.resolve()}\\static\\${imageNew}`);  // указываем нашу переменную imageNew,чтобы картинка правильно сохранилась и имела уникальное название
    ...
    async deleteProductCatalog(req, res, next) {
        ...
        try {
            ...
            // ищем все комментарии у которых productNameFor равен productCatalog.name (название объекта товара,который нужно удалить)
            const commentsForDeletedProduct = await commentModel.find({ productNameFor: productCatalog.name });
            ...
            if (commentsForDeletedProduct) {
                ...
                await commentModel.deleteMany({ productNameFor: productCatalog.name }); // удаляем все комментарии с помощью deleteMany(), у которых productNameFor равен productCatalog.name (название объекта товара,который нужно удалить)

            }

            const deletedProductsCart = await cartProductModel.find({ name: productCatalog.name }); // ищем все объекты товаров в корзине, у которых поле name равно productCatalog.name (название объекта товара,который нужно удалить)
            ...
            if (deletedProductsCart) {
                ...
                await cartProductModel.deleteMany({ name: productCatalog.name });  // удаляем все объекты товаров в корзине у которых поле name равно productCatalog.name (название объекта товара,который нужно удалить),указываем условие именно по полю name,так как нужно удалить все объекты этого товара у всех пользователей,потому что поле name у них одинаковое сохраняется в базе данных в таблице(сущности) cartProductModel,но разные id и forUser(это поле нужно,чтобы показать к какому пользователю этот товар принадлежит),поэтому так удаляем их у всех пользователей

            }

            const deletedProductCatalog = await productModel.findByIdAndDelete(productCatalog._id); // находим и удаляем объект товара каталога с помощью findByIdAndDelete(), у которого id равен productCatalog._id(указываем здесь нижнее подчеркивание перед id,так как оно по дефолту идет у объектов базы данных mongodb с нижним подчеркиванием)
            ...
            // проходимся по массиву descImages(массив названий картинок описания для товара),в descImages будут только названия файлов картинки,и на каждой итерации проверяем,есть ли уже такое название картинки как image в папке static,то есть есть ли такая картинка уже в папке static,и если хоть одна из них есть,то возвращаем ошибку,и дальше код выполняться не будет
            productCatalog.descImages.forEach(image => {
