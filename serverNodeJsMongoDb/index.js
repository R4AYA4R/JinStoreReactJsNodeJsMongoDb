...
const start = async () => {
    ...
    try {
        ...
        // создаем объект в базе данных в таблице(сущности) adminFieldsModel,в create указываем объект с полем phoneNumber и значением номера телефона в виде строки,создаем этот объект один раз,а потом этот код закомментируем,чтобы каждый раз этот объект не создавался,а только один раз,создаем этот объект,чтобы потом на фронтенде брать этот номер телефона из базы данных и чтобы потом админ мог его изменять, также создаем еще поле для визуального номера телефона(visible phoneNumber),как он будет выглядеть на сайте(админ сам будет это указывать),создаем это один раз,потом этот код закомментируем
        // await adminFieldsModel.create({ phoneNumber: "0800300353", visiblePhoneNumber: "0 800 300-353" });
